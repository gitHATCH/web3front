...
const Order = () => {
  ...
  const [orders,getOrders,loading,actualOrder,handleActualOrder,deleteOrder,editOrder,addOrder,addTara,turnBomb] = useContext(OrderContext)
  ...
  useEffect(() => {
    ...
    const masaActual = orders[actualOrder]?.ultimaCarga?.masa
    const estado = orders[actualOrder]?.estado
    const height = estado >= 3 ? 100 : (masaActual ? (masaActual*100/orders[actualOrder]?.preset) : 0)
        ...
        <div className="w-full flex justify-between items-center px-10">
          ...
          <div className=''>
            <button className='p-2 hover:bg-green-800 rounded-full mr-2'>
              {order.alarma ? (<svg xmlns="http://www.w3.org/2000/svg" onClick={() => toast.info("Ninguna notificación aún")} className='w-10 h-fit hover:opacity-70' viewBox="0 0 24 24"><title>bell</title><path d="M21,19V20H3V19L5,17V11C5,7.9 7.03,5.17 10,4.29C10,4.19 10,4.1 10,4A2,2 0 0,1 12,2A2,2 0 0,1 14,4C14,4.1 14,4.19 14,4.29C16.97,5.17 19,7.9 19,11V17L21,19M14,21A2,2 0 0,1 12,23A2,2 0 0,1 10,21" /></svg>) : (<svg xmlns="http://www.w3.org/2000/svg" className='w-10 h-fit hover:opacity-70 fill-red-700' onClick={() => checkAlarm()} viewBox="0 0 24 24"><title>bell-alert</title><path d="M23 7V13H21V7M21 15H23V17H21M12 2A2 2 0 0 0 10 4A2 2 0 0 0 10 4.29C7.12 5.14 5 7.82 5 11V17L3 19V20H21V19L19 17V11C19 7.82 16.88 5.14 14 4.29A2 2 0 0 0 14 4A2 2 0 0 0 12 2M10 21A2 2 0 0 0 12 23A2 2 0 0 0 14 21Z" /></svg>) }
            </button>
        ...
        <div className='flex flex-row justify-between p-10 w-fit items-center'>
          <div>
            ...
            <div className=' bg-slate-300 rounded-3xl shadow-2xl shadow-black p-10 items-center justify-center'>
              <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase'>Producto:<span className='ml-2 font-normal tracking-tight'>{order?.producto}</span></p>
              <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Camión: <span className='ml-2 font-normal tracking-tight'>{order?.camion}</span></p>
              <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Conductor: <span className='ml-2 font-normal tracking-tight'>{order?.chofer}</span></p>
              <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Preset: <span className='ml-2 font-normal tracking-tight'>{order?.preset}</span></p>
              <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Umbral Temperatura: <span className='ml-2 font-normal tracking-tight'>{order?.umbral}</span></p>

              {order?.estado >= 2 && (
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Pesaje Inicial: <span className='ml-2 font-normal tracking-tight'>AAA</span></p>
              )}
            </div>
          </div>
          {order?.estado == 1 && (
          ...
          {order?.estado >= 2 && order?.ultimaCarga && (
            <div className='ml-20'>
