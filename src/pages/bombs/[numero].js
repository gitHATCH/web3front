...
//TODO: Formatear y especificar eta y tiempo transcurrido

const Order = () => {
  const [order, setOrder] = useState({}) 
  const [load, setLoad] = useState(0) 
  const router = useRouter()
  const [orders,getOrders,loading,actualOrder,handleActualOrder,deleteOrder,editOrder,addOrder,addTara,turnBomb,getConcil,disableAlarm,changeUmbral,updateOrder] = useContext(OrderContext)
  ...
  return (
    <>
        ...
        <div className='flex flex-row justify-between p-10 w-fit items-center'>
          ...
          {actualOrder?.estado >= 2 && actualOrder?.ultimaMasa && (
            <div className='ml-20'>
              <h1 className='text-2xl mb-2 font-semibold'>Última Carga</h1>
              <div className=' bg-slate-300 rounded-3xl shadow-2xl shadow-black p-10 items-center justify-center'>
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase'>Masa:<span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimaMasa}</span></p>
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Temperatura: <span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimaTemperatura}</span></p>
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Densidad: <span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimaDensidad}</span></p>
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Caudal: <span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimoCaudal}</span></p>
                {/* <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>ETA: <span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimoEta}</span></p>
                <p className='text-2xl -tracking-tighter font-mono font-semibold uppercase mt-5'>Tiempo Transcurrido: <span className='ml-2 font-normal tracking-tight'>{actualOrder?.ultimoTiempoTransc}</span></p> */}
              </div>
            </div>
          )}
          {actualOrder?.estado == 2 && (
